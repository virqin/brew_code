...
#else
#define AEEFS_FILEMGR_DIR		"fs:/ftl1/"
#endif
			...
			if (AVK_SELECT == wParam || AVK_SOFT1 == wParam) 
			{
				...
				if (SUCCESS == IWIDGET_GetFocusIndex(me->piwList, &nIndex))
				{
					...
					else
					{
						...
						{
							SNPRINTF(szPath, sizeof(szPath), "%sPicture/Photo/%s", AEEFS_FILEMGR_DIR, szSection);
						}
						else
						{
							SNPRINTF(szPath, sizeof(szPath), "%sPicture/Photo/%s", AEEFS_CARD0_DIR, szSection);
						}
			...
			if (AVK_SELECT == wParam || AVK_SOFT1 == wParam) 
			{
				...
				if (SUCCESS == IWIDGET_GetFocusIndex(me->piwList, &nIndex))
				{
					...
					if (me->eStorageType == STOTAGE_PONE)
					{
						...
						{
							SNPRINTF(me->szSnapDir, sizeof(me->szSnapDir), "%sPicture/Photo/", AEEFS_FILEMGR_DIR);
						}
						else
						{
					...
					else
					{
						if (SUCCESS == IFILEMGR_Test(me->piFileMgr, AEEFS_CARD0_DIR))
						{
							SNPRINTF(me->szSnapDir, sizeof(me->szSnapDir), "%sPicture/Photo/", AEEFS_CARD0_DIR);
						}
...
static void CameraApp_ShowFolderList (void * pUser)
{
	...
	if (me->eStorageType == STOTAGE_PONE)
	{
		SNPRINTF(szPath, sizeof(szPath), "%sPicture/Photo", AEEFS_FILEMGR_DIR);
	}
	else
	{
		SNPRINTF(szPath, sizeof(szPath), "%sPicture/Photo", AEEFS_CARD0_DIR);
	}
...
static int CameraApp_CreateWidgets(CameraApp *me)
{
	...
	DBGPRINTF("CameraApp_CreateWidgets in");
   ...
   if (!nErr) {
	...
	if (!nErr)
	{
		...
		IVALUEMODEL_SetValue(me->piValueModel, (void *)-1, 4, NULL);
	}
...
static int CameraApp_Start(CameraApp *me)
{
	...
	DBGPRINTF("CameraApp_Start in");

	nErr = CameraApp_CreateWidgets(me);
	DBGPRINTF("CameraApp_CreateWidgets Over! nErr:%d", nErr);
	if (!nErr)
	{
...
static int CameraApp_Construct(CameraApp *me, IModule * piModule, IShell * piShell)
{
	...
	DBGPRINTF("CameraApp_Construct in");
	...
	if (!nErr) {
		...
		DBGPRINTF("CameraApp_Construct AEECLSID_ROOTCONTAINER (%d,%d) nErr:%d", biDevice.cx, biDevice.cy, nErr);
	}
	...
	if (!nErr) {
		...
		DBGPRINTF("CameraApp_Construct AEEIID_WIDGET nErr:%d", nErr);
		IWIDGET_SetBGColor(me->piwRoot, MAKE_RGB(25,25,25));
	}
	...
	DBGPRINTF("CameraApp_Construct in2 nErr:%d", nErr);
	...
	DBGPRINTF("CameraApp_Construct in3");
	...
	if (!nErr) {
		...
		size = (WSTRLEN((const AECHAR *)wstr)+6)*sizeof(char);
		WSTRTOSTR((const AECHAR *)wstr, szBuf, size);
	...
	DBGPRINTF("CameraApp_Construct in4");
	...
	DBGPRINTF("CameraApp_Construct in5");
	...
	DBGPRINTF("CameraApp_Construct over");
...
{
	DBGPRINTF("%s %x in", "xModule_CreateInstance", idClass);
