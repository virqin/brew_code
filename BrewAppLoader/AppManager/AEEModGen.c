...
static void       AEEMod_SaveTempData(IShell *pIShell, void *ph);
...
{
   //void* pData = MALLOC(387000);
   AEEMod_SaveTempData(pIShell,ph);
...
static void AEEMod_SaveTempData(IShell *pIShell, void *ph)
{
	...
	{
		IFILEMGR_Remove(pFm,AEEFS_HOME_DIR"temp.data");
	}
...
static uint32 AEEMod_AddRef(IModule *po)
{
	//DBGPRINTF("****AppManager AEEMod_AddRef:%d********************",((((AEEMod *)po)->m_nRefs) + 1));
...
static uint32 AEEMod_Release(IModule *po)
{
   ...
   //DBGPRINTF("****AppManager AEEMod_Release:%d********************",pMe->m_nRefs);
   if (--pMe->m_nRefs != 0) {
      return pMe->m_nRefs;
   }
