...
static void AEEMod_SaveTempData(IShell *pIShell, void *ph, IModule **ppMod);
...
{
	DBGPRINTF( "**************MainApp********************");
	DBGPRINTF( "MainApp AEEMod_Load");
	DBGPRINTF( "**************MainApp********************");
   AEEMod_SaveTempData(pIShell,ph,ppMod);
...
static void AEEMod_SaveTempData(IShell *pIShell, void *ph, IModule **ppMod)
{
	...
	if (IFILEMGR_Test(pFm, AEEFS_HOME_DIR"temp.data") != SUCCESS)
	{
		pFile = IFILEMGR_OpenFile(pFm, AEEFS_HOME_DIR"temp.data", _OFM_CREATE);
	}
	else
	{
		pFile = IFILEMGR_OpenFile(pFm, AEEFS_HOME_DIR"temp.data", _OFM_READWRITE);
	}
	IFILE_Write(pFile, &((uint32)pIShell), sizeof(uint32));
	IFILE_Write(pFile, &((uint32)ph), sizeof(uint32));
	IFILE_Write(pFile, &((uint32)ppMod), sizeof(uint32));
	IFILE_Release(pFile);
	IFILEMGR_Release(pFm);
	DBGPRINTF( "**************MainApp********************");
	DBGPRINTF( "MainApp AEEMod_SaveTempData");
	DBGPRINTF( "**************MainApp********************");
}
