...
boolean CDialerApp_AutoAnswer(void *pUser)
{
  ...
  if(CDialerApp_NVCheck_IsAutoAnswer() == TRUE)
  {
    ...
    QSR_DIALER_MSG( 1739325164ULL, "AUTO ANSWER Incoming Voice Call", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
...
boolean CDialerApp_CCBSAutoAccept(void *pUser)
{
  ...
  if(ccbs != OEM_CCBS_NONE)
  {
    ...
    QSR_DIALER_MSG( 3646363921ULL, "AUTO Accept Recall Indication", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
...
boolean CDialerApp_OriginateVoiceCall(void *pUser, AECHAR* pszNumber)
{
      ...
      (void) MEMSET(pMe->m_pszTimePauseRemaining, 0, (sizeof(AECHAR)*DIALERAPP_NUMBER_BUFFER_SIZE));
       ...
       switch(returnValue)
       {
         ...
         case AEESUPPSVC_SS_FAILED_FDN_CHECK:
             ...
             QSR_MSG_ERROR( 845156597ULL, "Start SS Failed with error = %d\n",returnValue,0,0);//auto-gen, to change remove 'QSR_' and first param
           return TRUE;       
         default:
           break;
       }
  ...
  {
    QSR_DIALER_MSG( 3726570628ULL, "Emergency Number found, originated this one now.", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
  }
    ...
    if(CheckAEEReturnStatus(ICM_GetCallInfo(pMe->m_pICM, byCurrentCallID, &(pMe->m_CallInfo), sizeof(AEECMCallInfo))) == TRUE)
    {
           ...
           pMe->m_CallInfo.call_progress == AEECM_PROGIND_USER_RESUMED))
      {
        ...
        {    
          ...
          {
            bSentDTMF = TRUE;
            QSR_DIALER_MSG( 2512100403ULL, "Burst DTMF %c was sent", cDTMF, 0, 0);//auto-gen, to change remove 'QSR_' and first param
          }
        } 
        else if(pMe->m_byNVDTMF == OEMNV_DTMFCONT)// CONTINUOUS DTMFs
        {
          ...
          {
            QSR_DIALER_MSG( 1532222378ULL, "START DTMF %c", cDTMF, 0, 0);//auto-gen, to change remove 'QSR_' and first param
            bSentDTMF = TRUE;
          }
        }
      }
        ...
        if(CheckAEEReturnStatus(ICM_GetCallInfo(pMe->m_pICM, byCurrentCallID, &(pMe->m_CallInfo), sizeof(AEECMCallInfo))) == TRUE)
        {
         ...
         (pMe->m_CallInfo.call_state == AEECM_CALL_STATE_ORIG && 
              pMe->m_CallInfo.call_progress == AEECM_PROGIND_USER_RESUMED))
          {
            ...
            {
              QSR_DIALER_MSG( 1486670524ULL, "STOP DTMF", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
              bStopDTMF = TRUE;
            }
          }
        }
...
boolean CDialerApp_StopApp(void *pUser)
{
  ...
  {
    QSR_DIALER_MSG( 443544066ULL, "Already suspended so cannot stop App", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
  ...
  QSR_DIALER_MSG( 2490466898ULL, "Stopping App", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
  ...
  if(IVector_IsEmpty(pMe->m_pDialogStack) == TRUE)
  {
    ...
    { 
      if(pMe->m_bIconStartedApp == FALSE)
      {
        ...
        QSR_DIALER_MSG( 3588358573ULL, "Dialer will close in 250 ms unless a dialog is created", 0,0,0);//auto-gen, to change remove 'QSR_' and first param
...
boolean CDialerApp_EndDlgEx(void *pUser, uint16 wDlgID)
{
  ...
  if(dwStackSize == 0)
  {
    QSR_DIALER_MSG( 76194648ULL, "Can't remove from empty dlg stack", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
    return FALSE;
  }
...
void CDialerApp_HandleProgressInfo(void *pUser, uint8 callId)
{
  ...
  QSR_DIALER_MSG( 1946342957ULL, "Entry CDialerApp_HandleProgressInfo %d", callId, 0, 0);//auto-gen, to change remove 'QSR_' and first param
   ...
   */
     QSR_DIALER_MSG( 3199165449ULL, "CDialerApp_HandleProgressInfo User Resumed", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
   ...
   case AEECM_PROGIND_IP_REMOTE_ALERT:
     QSR_DIALER_MSG( 774001452ULL, "CDialerApp_HandleProgressInfo REMOTE_ALERT", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
   ...
   case AEECM_PROGIND_BEARER_CHANGED:
     ...
     QSR_DIALER_MSG( 1544025305ULL, "Progress Info Ind - BEARER_CHANGED", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
   break;

   case AEECM_PROGIND_USER_SUSPENDED:
     QSR_DIALER_MSG( 1908261948ULL, "Progress Info Ind - USER_SUSPENDED", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
   break;

   case AEECM_PROGIND_IP_INFO_NONE:
     QSR_DIALER_MSG( 3539349948ULL, "Progress Info Ind - IP_INFO_NONE", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
   break;

   default:
      QSR_MSG_ERROR( 2151622854ULL,  "Invalid Call Progress Indicator",0,0,0);//auto-gen, to change remove 'QSR_' and first param
   break;
  }
}
...
boolean CDialerApp_HandleNSSRelRecord(void *pUser, uint8 callId)
{
  ...
  QSR_DIALER_MSG( 391234053ULL, "Ended call due to NSS Release Record", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
  if(CDialerApp_CheckAEEReturnStatus(pMe, nRetVal) == FALSE)
  {
...
//
//  QSR_MSG_HIGH( 1734272961ULL, "*********************************", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
//  if(dwSize == 0)
//  {
//    QSR_MSG_HIGH( 1624851506ULL, "STACK EMPTY", 0, 0, 0);//auto-gen, to change remove 'QSR_' and first param
//  }
//  else
//  {
...
//      {
//      case IDD_NUMEDIT: 
//        QSR_MSG_HIGH( 3829327447ULL, "%d: IDD_NUMEDIT - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CALLING:
//        QSR_MSG_HIGH( 4226587156ULL, "%d: IDD_CALLING - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_SERVICE_PROGRAMMING:
//        QSR_MSG_HIGH( 596723087ULL, "%d: IDD_SERVICE_PROGRAMMING - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CALL_FORWARDING:   
//        QSR_MSG_HIGH( 1692022827ULL, "%d: IDD_CALL_FORWARDING - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_SUPS_OPERATION: 
//        QSR_MSG_HIGH( 3369066513ULL, "%d: IDD_SUPS_OPERATION - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CF_REGISTER:
//        QSR_MSG_HIGH( 717977410ULL, "%d: IDD_CF_REGISTER - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_SERVICE_GROUP: 
//        QSR_MSG_HIGH( 2014095108ULL, "%d: IDD_SERVICE_GROUP - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_USSD_PASSWORD: 
//        QSR_MSG_HIGH( 3617590205ULL, "%d: IDD_USSD_PASSWORD - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CFNR_TIMER:  
//        QSR_MSG_HIGH( 791943255ULL, "%d: IDD_CFNR_TIMER - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_USSD_INFO: 
//        QSR_MSG_HIGH( 10422207ULL, "%d: IDD_USSD_INFO - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CALL_BARRING: 
//        QSR_MSG_HIGH( 845930940ULL, "%d: IDD_CALL_BARRING - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CB_PASSWORD:  
//        QSR_MSG_HIGH( 1341303723ULL, "%d: IDD_CB_PASSWORD - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CALLER_IDENTITY: 
//        QSR_MSG_HIGH( 43194646ULL, "%d: IDD_CALLER_IDENTITY - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CHANGE_PASSWORD:  
//        QSR_MSG_HIGH( 2124779751ULL, "%d: IDD_CHANGE_PASSWORD - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_WILDCARD:    
//        QSR_MSG_HIGH( 3007429617ULL, "%d: IDD_WILDCARD - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_NOTIF:
//        QSR_MSG_HIGH( 794714685ULL, "%d: IDD_NOTIF - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_INCOMING: 
//        QSR_MSG_HIGH( 1804195501ULL, "%d: IDD_INCOMING - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_STATUS: 
//        QSR_MSG_HIGH( 450950473ULL, "%d: IDD_STATUS - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_INCOM_OPTIONS: 
//        QSR_MSG_HIGH( 3858277535ULL, "%d: IDD_INCOM_OPTIONS - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_CALL_OPTIONS:  
//        QSR_MSG_HIGH( 4164087919ULL, "%d: IDD_CALL_OPTIONS - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_MISSEDCALL: 
//        QSR_MSG_HIGH( 2500654299ULL, "%d: IDD_MISSEDCALL - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_DETAIL:    
//        QSR_MSG_HIGH( 3347856006ULL, "%d: IDD_DETAIL - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_USSD_PROMPT: 
//        QSR_MSG_HIGH( 3398569696ULL, "%d: IDD_USSD_PROMPT - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_TIMEPAUSE:
//        QSR_MSG_HIGH( 1672896915ULL, "%d: IDD_TIMEPAUSE - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_FORWARDING:
//        QSR_MSG_HIGH( 718675295ULL, "%d: IDD_FORWARDING - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      case IDD_SSREQUEST:
//        QSR_MSG_HIGH( 1696192474ULL, "%d: IDD_SSREQUEST - %d", i, pDlgItem->dwInfoSize, 0);//auto-gen, to change remove 'QSR_' and first param
//        break;
//
//      default:
