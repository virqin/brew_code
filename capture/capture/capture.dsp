...
SOURCE=.\res\capture.brx

!IF  "$(CFG)" == "capture - Win32 Release"

# Begin Custom Build
InputPath=.\res\capture.brx

BuildCmds= \
	"$(BREWSDKTOOLSDIR)\ResourceEditor\brewrc.exe" -s -o capture.bar -h capture_res.h $(InputPath)

"capture.bar" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)

"capture_res.h" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)
# End Custom Build
...
BuildCmds= \
	"$(BREWSDKTOOLSDIR)\ResourceEditor\brewrc.exe" -s -o ./capture.bar -h ./inc/capture_res.h $(InputPath)
...
SOURCE=.\mif\capture.mfx

!IF  "$(CFG)" == "capture - Win32 Release"

# Begin Custom Build
TargetName=capture
InputPath=.\mif\capture.mfx
...
"capture.mif" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
	"$(BREWSDKTOOLSDIR)\ResourceEditor\brewrc.exe" -nh  -o ..\capture.mif -i mif $(InputPath)
