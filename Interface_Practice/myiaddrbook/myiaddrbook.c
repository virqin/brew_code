			...
			case IDS_DEL_REC:
				...
				if(nRet == AEE_SUCCESS)
				{
					...
					if(pRec)
					{
						...
						if(IADDRREC_RemoveRec(pRec) == AEE_SUCCESS)
							ISHELL_LoadResString(pMe->a.m_pIShell, MYIADDRBOOK_RES_FILE, IDS_DEL_REC_OK, szText, sizeof(szText));
						else
							ISHELL_LoadResString(pMe->a.m_pIShell, MYIADDRBOOK_RES_FILE, IDS_DEL_REC_FAIL, szText, sizeof(szText));
					}
					else
						ISHELL_LoadResString(pMe->a.m_pIShell, MYIADDRBOOK_RES_FILE, IDS_LOCATE_ERR, szText, sizeof(szText));
				}
